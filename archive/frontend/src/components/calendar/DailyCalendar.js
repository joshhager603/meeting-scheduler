...
const DailyCalendar = () => {
    ...
    return (
        <div>
            <div>
                <div className="font-bold text-xl text-gray-800">{selectedCalendar?.title}</div>
                <div className="text-gray-600 text-base mt-2">{selectedCalendar?.details}</div>
            </div>
            <br/>
                                    ...
                                    <ul className="list-disc list-inside">
                                        {meeting.participants.map((participant, index) => (
                                            <li key={index} className="text-gray-700">
                                                {participant.name} - <a href={`mailto:${participant.email}`}
                                                                        className="text-blue-500">{participant.email}</a>
                                            </li>
                                        ))}
                                    </ul>
                                ) : (
                                        ...
                                        {meeting.attachments.map((attachment, index) => (
                                            <li key={index} className="text-gray-700">
                                                <a target='_blank' href={`${attachment.url}`}
                                                   className="text-blue-500">{attachment.url}</a>
                                            </li>
                                        ))}
                                    </ul>
                                ) : (
